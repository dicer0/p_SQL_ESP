...
/*3.	¿Existe alguna etiqueta que no corresponda a ningún post?
      ...
      b.	Los datos los voy a tomar de la tabla de las etiquetas, pero como quiero saber su conexión con post, no es necesario que 
          analice post, solo la tabla de etiquetas y su tabla de transición intermedia.
          i.	Debido a esta situación se deberá ejecutar un LEFT JOIN (de la tabla etiquetas), doble se considere solo las etiquetas 
              que no tengan conexión, osea A – B, siendo A = etiquetas y B = tabla_intermedia_con_conexión_a_posts.
...
/*4.	¿Cuál categoría posee un mayor número de posts?
      ...
      b.	La información proviene de 2 tablas distintas: posts y etiquetas.
          i.	Como se busca encontrar los datos relacionados se deberá ejecutar un INNER JOIN que considere la intersección de las 
              2 tablas, osea A ∩ B, recordemos que esto se logra al utilizar el índice que relaciona ambas tablas.
      c.	La información se agrupa a través del id de la categoría porque de esa manera se podrá mostrar cada tipo de categoría 
          distinta.
...
/*5.	¿Qué usuario ha creado el mayor número de posts en el sistema?
      a.	El procedimiento es el mismo al ejercicio anterior, pero cambiando la tabla de donde provienen los datos y el dato mismo 
          que se quiere mostrar.
      b.	Opcionalmente nos podemos limitar a mostrar 1 solo valor, de esta manera mostrando solo el mayor o los primeros dos, para 
          comprobar si en el segundo se repite el número.*/
...
/*6.	¿De qué categorías (temas) están escribiendo los 3 usuarios que han creado el mayor número de posts en el sistema?
      ...
      b.	Como la información proviene de 3 tablas distintas: usuarios, categorías y posts, se debe realizar una interconexión de 
          ...
          i.	Debido a que se están buscando los datos que pertenezcan a las 3 tablas a la vez, se deberá ejecutar un INNER JOIN 
              doble que considere la intersección de las 3 tablas osea A∩B∩C, recordemos que esto se logra al utilizar el índice 
...
/*7.	¿Qué usuarios no han escrito ningún post?
      ...
      b.	Los datos los voy a tomar de la tabla de los usuarios y de los posts.
          i.	Debido que quiero saber todos los usuarios que no tengan ningún post se ejecutará una operación de LEFT JOIN, doble 
              se considere solo los usuarios que no tengan conexión, osea A – B, siendo A = usuarios y B = posts.
